"""
Command Core — Streamlit UI v3 (Corporate Light Tactical)
==========================================================
...
To run: streamlit run ui/streamlit_app_v3.py
"""
...
.left-panel::-webkit-scrollbar-track { background: #F4F6F9; }
...
.main-area::-webkit-scrollbar-track { background: #F8FAFC; }
...
/* ── New incident button ── */
.stButton > button[data-testid="new-incident-btn"] {
...
.stButton > button[data-testid="new-incident-btn"]:hover {
...
.veto-tbl tr:hover td { background: #F8FAFC; }
...
/* ── Text area & input overrides for light theme ── */
.stTextArea textarea {
    background: #FFFFFF !important;
    color: #1E293B !important;
    border: 1.5px solid #CBD5E1 !important;
...
with left_panel:
    ...
    # ── Constitution Rules (collapsible) ──
    ...
    # ── Veto Trigger Manager (collapsible) ──
    with st.expander("🔍 Veto Trigger Manager", expanded=False):
        ...
        new_phrase = st.text_input("Phrase", key="new_trigger_phrase", placeholder="e.g. unstable roof")
        new_rule   = st.text_input("Rule label", key="new_trigger_rule", placeholder="e.g. RULE 2 — Structural hazard")
        ...
        edited_triggers = st.text_area(
            ...
            height=160,
            label_visibility="collapsed",
            key="triggers_json_editor",
        )
        if st.button("💾 Save JSON", use_container_width=True, key="save_triggers_json"):
            try:
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["txt", "md"],
        label_visibility="collapsed",
        key="file_uploader_v3",
    )
    if uploaded_file is not None:
        file_bytes = uploaded_file.read()
        if len(file_bytes) > MAX_UPLOAD_BYTES:
            size_mb = len(file_bytes) / (1024 * 1024)
            st.error(f"❌ File too large ({size_mb:.2f} MB). Max {MAX_UPLOAD_BYTES // (1024*1024)} MB.")
        else:
            file_content = file_bytes.decode("utf-8", errors="replace")
            if st.button("🚀 Process Report", use_container_width=True, key="process_upload_btn"):
...
with main_area:
    ...
    st.markdown('<div class="stage-title">📡 Field Report Dispatch</div>', unsafe_allow_html=True)
    ...
    # =========================================================================
    st.markdown('<div class="stage-card">', unsafe_allow_html=True)
    st.markdown('<div class="stage-title">🤖 Parallel Agent Analysis</div>', unsafe_allow_html=True)
    ...
    if result and result.get("specialists"):
        # Always show in consistent order: Fire, Med, Police
    ...
    # =========================================================================
    # STAGE 3 — Unified Command Decision
    # =========================================================================
    ...
    if result and result.get("final_plan"):
        ...
        st.markdown(
            '<div class="empty-state" style="background:#FFFFFF;">'
    ...
    # ── Veto Audit Log ────────────────────────────────────────────────────────
    st.markdown('<div class="stage-card">', unsafe_allow_html=True)
    st.markdown('<div class="stage-title">📋 Veto Audit Log</div>', unsafe_allow_html=True)
    ...
    if conflicts:
        st.markdown('<div class="stage-card">', unsafe_allow_html=True)
        st.markdown('<div class="stage-title">⚔️ Conflict Resolution</div>', unsafe_allow_html=True)
    ...
    # Footer
    st.markdown(
        f'<div style="font-size:11px;color:#94A3B8;padding:10px 0;border-top:1px solid #E2E8F0;margin-top:8px;">'
